...
rule "terraform_documented_variables" {
  enabled = false # Example variables.tf is empty by request
}

rule "terraform_documented_outputs" {
  enabled = false # Example outputs.tf is empty by request
}

# Code Quality Rules
rule "terraform_typed_variables" {
  enabled = true # Should still apply if any variables were defined
}
